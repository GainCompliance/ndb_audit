...
class NDBAuditUnitTest(NDBUnitTest):
    ...
    def test_blind_put(self):
        for cls in self._TEST_CLASSES:
            ...
            ent1 = cls(key=fookey, foo='a', bar=1)
            self._trans_put(ent1)
            self.assertEqual(ent1.data_hash, hashlib.sha1('{v1}bar=1|foo=a').hexdigest()[0:16])
            ent2 = cls(key=fookey, foo='b', bar=2)
            self._trans_put(ent2)
            self.assertEqual(ent2.data_hash, hashlib.sha1('{v1}bar=2|foo=b').hexdigest()[0:16])
    ...
    def test_put_when_data_unchanged(self):
        ...
        for cls in self._TEST_CLASSES:
            ...
            ent1.bar = 2
            self._trans_put(ent1)
            new_data_hash = hashlib.sha1('{v1}bar=2|foo=b').hexdigest()[0:16]
            def check_v2():
